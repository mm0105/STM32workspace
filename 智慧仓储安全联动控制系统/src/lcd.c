 ...
 * 返 回 值: 无
 ***************************************************************/
static void lcd_show_chinese_12x12(uint16_t x, uint16_t y, const uint8_t *s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    uint8_t i, j, m=0;
    uint16_t k;
    uint16_t HZnum;//汉字数目
 ...
 * 返 回 值: 无
 ***************************************************************/
static void lcd_show_chinese_16x16(uint16_t x, uint16_t y, const uint8_t *s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    uint8_t i, j, m = 0;
    uint16_t k;
    uint16_t HZnum;//汉字数目
 ...
 * 返 回 值: 无
 ***************************************************************/
static void lcd_show_chinese_24x24(uint16_t x, uint16_t y, const uint8_t *s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    uint8_t i,j,m = 0;
    uint16_t k;
    uint16_t HZnum;//汉字数目
 ...
 * 返 回 值: 无
 ***************************************************************/
static void lcd_show_chinese_32x32(uint16_t x, uint16_t y, const uint8_t *s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    uint8_t i,j,m = 0;
    uint16_t k;
    uint16_t HZnum;//汉字数目
 ...
 * 返 回 值: 无
 ***************************************************************/
void lcd_show_chinese(uint16_t x, uint16_t y, const char *s,
    uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
    ...
    for (uint32_t i = 0; i < strlen(s); i += 3, x += sizey)
    {
        if (sizey == 12)
        {
            lcd_show_chinese_12x12(x, y, (const uint8_t *)&s[i], fc, bc, sizey, mode);
        }
        else if (sizey == 16)
        {
            lcd_show_chinese_16x16(x, y, (const uint8_t *)&s[i], fc, bc, sizey, mode);
        }
        else if (sizey == 24)
        {
            lcd_show_chinese_24x24(x, y, (const uint8_t *)&s[i], fc, bc, sizey, mode);
        }
        else if (sizey == 32)
        {
            lcd_show_chinese_32x32(x, y, (const uint8_t *)&s[i], fc, bc, sizey, mode);
        }
        else
        {
            return;
...
void lcd_show_text(int x, int y, const char *str, int fc, int bc, int font_size, int mode)
{
    const unsigned char *tmp_str = (const unsigned char *)str;
    int cur_x=x;
    int cur_y=y;
    ...
    {
        if(*tmp_str < 0x80){
            /* ASCII - 走原字符渲染(有正确的 ascii_xx 字模) */
            lcd_show_char(cur_x, cur_y, *tmp_str, fc, bc, font_size, mode);
            tmp_str++;
            cur_x += font_size/2;
        } else {
            /* UTF-8 中文字符: 3 字节一汉字
             *  -> 调 lcd_show_chinese(底层 tfont 数组 Index 是 UTF-8 3 字节) */
            if((*tmp_str & 0xF0) == 0xE0) {
                uint8_t utf8[4] = {tmp_str[0], tmp_str[1], tmp_str[2], 0};
                lcd_show_chinese(cur_x, cur_y, (const char *)utf8,
                                 fc, bc, font_size, mode);
                tmp_str += 3;
                cur_x += font_size;
            } else if((*tmp_str & 0xF0) == 0xF0) {
                /* 4 字节 UTF-8 罕用字符, 跳过 */
                tmp_str += 4;
                cur_x += font_size;
            } else {
