...
#define HOST_ADDR "your-iotda-host"

#define CLIENT_ID "your-client-id"
#define DEVICE_ID  "your-device-id"
#define MQTT_DEVICES_PWD "your-mqtt-password"
...
void send_msg_to_mqtt(e_iot_data *iot_data) {
  ...
  if (root != NULL) {
    ...
    // 气体浓度
    sprintf(str, "%5.2fppm", iot_data->gas_ppm);
    cJSON_AddStringToObject(pro_obj, "gas", str);
    // 火灾状态
    if (iot_data->fire_state == true) {
      cJSON_AddStringToObject(pro_obj, "fireStatus", "ON");
    } else {
      cJSON_AddStringToObject(pro_obj, "fireStatus", "OFF");
    }
    // 电机状态
